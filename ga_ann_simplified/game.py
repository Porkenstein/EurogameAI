...
WEALTH_MAX = 30
BUILDING_MAX = 5 # shouldn't have more than this many work spaces of type
ABUNDANCE_MAX = 12
PRODUCTION_MAX = 12
PLANTATION_MAX = 12
AVAILABILITY_MAX = 6
...
UNEMPLOYED_MAX = 10
NUM_ENV    = 10
ITER_COUNT_MAX = 50 # if the AI gets obstinant
BLANK_MAX = 15 # there should never be this many blank spaces
...
class Game:    
    ...
    def get_game_state(self, player):  # new, abstracted game state
        state = []
        state.append(log((min(WEALTH_MAX, self.gold[player])/WEALTH_MAX) * 9 + 1, 10)) # player gold
        for i in range(4, 6): # the number of non-production work spaces of each type
            state.append(log((min(BUILDING_MAX, self.bonus(player, BID(i), True))/BUILDING_MAX) * 9 + 1, 10))
        #markets
        state.append(log((min(BUILDING_MAX, self.bonus(player, BID.small_market, True) + self.bonus(player, BID.large_market, True))/BUILDING_MAX) * 9 + 1, 10))
        for i in range(9, 11):
            state.append(log((min(BUILDING_MAX, self.bonus(player, BID(i), True))/BUILDING_MAX) * 9 + 1, 10))
        #warehouses
        state.append(log((min(BUILDING_MAX, self.bonus(player, BID.small_warehouse, True) + self.bonus(player, BID.large_warehouse, True))/BUILDING_MAX) * 9 + 1, 10))
        ...
        for i in range(1, 5): # amount of building production available per crop type
        ...
        state.append(log((min(BLANK_MAX, self.cities[player].get_blank_spaces(False))/BLANK_MAX) * 9 + 1, 10)) # number of unworked work spaces
        self.last_game_state = state
        return state
        
    # def get_game_state(self, player):  old, full game state
